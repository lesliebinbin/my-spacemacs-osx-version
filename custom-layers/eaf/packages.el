...
(defun eaf/init-eaf ()
  (use-package eaf
    ...
    :init (progn
            ...
            (setq eaf-browser-keybinding '(("C--" . "zoom_out")
                                           ("C-=" . "zoom_in")
                                           ("C-0" . "zoom_reset")
            ...
            (setq eaf-pdf-viewer-keybinding '(("j" . "scroll_up")
                                              ("<down>" . "scroll_up")
                                              ("C-n" . "scroll_up")
                                              ("k" . "scroll_down")
                                              ("<up>" . "scroll_down")
...
;;;; TODO need to consider the current pdf view mode which does not need to be pdf view mode
      (spacemacs/set-leader-keys-for-major-mode
        'pdf-view-mode "E" 'spacemacs/open-with-eaf)
  ...
  (use-package eaf-evil
    ...
    :config (progn
              ...
              (define-key key-translation-map (kbd "SPC") (lambda (prompt)
                                                            (if (derived-mode-p 'eaf-mode)
                                                                (pcase eaf--buffer-app-name
                                                                  ((or (and "browser"
                                                                            (guard (not (string= (eaf-call-sync "call_function" eaf--buffer-id
                                                                                                                "is_focus")
                                                                                                 "True"))))
                                                                       "image-viewer"
                                                                       "pdf-viewer")
              ...
              (define-key key-translation-map (kbd ",") (lambda (prompt)
                                                          (if (derived-mode-p 'eaf-mode)
                                                              (if (string= (eaf-call-sync "call_function" eaf--buffer-id
                                                                                          "is_focus")
                                                                           "True")
                                                                  (kbd ",")
                                                                (kbd "C-,"))))))))
